...
# Поскольку матрицы --- это двумерные объекты, первым в квадратных скобках следует номер строки, вторым --- номер столбца
...
# Функции для конверсии на `as...()`. Функции для проверки на `is...()`
...
# В пакете `readxl` используется более совершенный алгоритм автоматического определения классов переменных.
sapply(dat3, class)
sum(is.na(dat3$`2`))
sum(is.na(dat3$`6`))
