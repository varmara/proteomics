...
```{r power_data, echo=FALSE, cache=TRUE, message=FALSE}
...
```{r power, echo=FALSE, fig.height=1.5, fig.width=4}
...
- __Модерированный t-тест__ (с использованием Empirical Bayes) --- более мощный, чем обычный t-критерий. Позволяет точнее оценить $\sigma$ для конкретного пептида, используя информацию о распределении $\sigma$ для всех пептидов.
...
Для сегодняшнего занятия давайте возьмем только гребешков из 10 градусов
...
Давайте сравним уровень экспрессии первого пептида между группами при помощи простого t-критерия.
...
Но у нас всего `r nrow(expr_log)` пептидов. Было бы не удобно делать все эти сравнения вручную. Поэтому, давайте научимся добывать из объекта, возвращаемого `t.test()` значение p-value.
...
Теперь мы готовы посчитать t-тест для каждого пептида. Для этого нам понадобится:
...
Все готово, мы посчитали p-values для всех пептидов. 
...
rownames(expr_log)[ids_dif]
```

Проблема в том, что __это пока еще не правильные p-values!__
...
__Поправка Бонферрони__ --- процедура в один шаг. Отклоняем все нулевые гипотезы, для которых $p \le \frac {\alpha} {m}$. Например, если вам нужно сделать 1000 сравнений, чтобы вероятность совершить ошибку I рода была 0.05, то нужно использовать $p = 0.05/1000 = 0.00005$ для каждого сравнения. 
...
Например, в случае 15 сравнений для каждого из p-values мы бы получили свой порог значимости: 
...
sim_pval <- mapply(
  FUN = runif, n = 1,
  min = c(0.0001, 0.001, 0.005, 0.01, 0.05,
    seq(0.1, to = 0.9, by = 0.1), 1), 
  max = c(0.001, 0.005, 0.01, 0.05, 0.1, 
    seq(0.2, to = 1, by = 0.1), 1))
n <- length(sim_pval)

dat <- data.frame(rank = rank(sim_pval), pval = round(sim_pval, 4))

adj_holm <- function(p, j, n){
  (n - j + 1)
}

# adj_holm(dat$pval, dat$rank, n)

dat$adj <- adj_holm(dat$pval, dat$rank, n)

# sapply(dat$adj*dat$pval, min, 1)

dat$padj <- mapply(FUN = min, dat$adj*dat$pval, 1)
dat$h0 <- ifelse(dat$padj <= 0.05, "Да", "Нет")
...
__Частота ложноположительных результатов (false discovery rate, FDR)__ --- это доля ошибок I рода относительно общего числа отвегнутых $H_0$. Для
...
__Задание.__ Посчитайте пожалуйста самостоятельно, сколько достоверно различающихся пептидов будет найдено после поправки Хольма и после применения процедуры Беньямини-Хохберга.

### 2.Распределение t-статистики
...
При рассчете обычного t-критерия для оценки дисперсии экспрессии пептидов используется выборочная оценка дисперсии. Если повторностей мало - оценки дисперсии получаются нестабильными и не точными. t-критерий лучше работает при __большом__ числе повторностей, т.к. удается точнее оценить $\sigma$
